...
class Graph {
  ...
  Graph(int verticesCount) {
    this.totalVertices = verticesCount;
    colors = new int[verticesCount];
  }

  boolean isGraphColorable(int[][] graph, int numberOfColors) {
  ...
  boolean graphColoringUtils(int[][] graph, int numberOfColors, int[] colors, int currentVertex) {
    if(currentVertex == totalVertices) return true;
  ...
  private boolean isSafeToAllotColor(int[][] graph, int[] colors, int currentColor, int currentVertex) {
    for(int i = 0; i < totalVertices; i++)
      if(graph[currentVertex][i] == 1 && colors[i] == currentColor) return false;
    return true;
  }
