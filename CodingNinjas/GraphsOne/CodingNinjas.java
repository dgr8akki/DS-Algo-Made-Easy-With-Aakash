...
public class CodingNinjas {
  ...
  private boolean isValid(
    int x, int y, boolean[][] isVisited, int N, int M, String[] cake, int currentCharacterIndex) {
    return currentCharacterIndex < cn.length()
        && x >= 0
        && x < N
        && y < M
        && y >= 0
        && cake[x].charAt(y) == cn.charAt(currentCharacterIndex)
        && !isVisited[x][y];
  }
}
