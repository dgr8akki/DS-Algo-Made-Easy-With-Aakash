public class EggDropping {
    ...
    for(int eggIndex = 2; eggIndex <= totalEggs; eggIndex++) {
      for(int floorIndex = 1; floorIndex <= totalFloors; floorIndex++) {
        ...
        for(int k = 1; k <= floorIndex; k++) {
          attempt = 1 + Math.max(attempts[eggIndex - 1][k - 1], attempts[eggIndex][floorIndex - k]);

          if(attempt < attempts[eggIndex][floorIndex]) {
            attempts[eggIndex][floorIndex] = attempt;
          }
        }
      }
    }
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Minimum " + eggDropping.calculateAttempts(2, 100) + " attempts needed, if you have 2 eggs and 100 floor building.");
  }
}