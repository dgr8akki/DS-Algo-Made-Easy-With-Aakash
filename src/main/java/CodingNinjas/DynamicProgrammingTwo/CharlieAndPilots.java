...
public class CharlieAndPilots {
  ...
  private static int solve(
    int n, int[] captainSalaries, int[] assistantSalaries, int[] sumOfCaptainSalaries) {
    // return assistantSalaries[0] + charlieAndPilotsRecursive(n - 1, captainSalaries,
    // assistantSalaries, 1, 1);
    return charlieAndPilotsIterative(n, captainSalaries, assistantSalaries, sumOfCaptainSalaries);
  }

  private static int charlieAndPilotsIterative(
    int n, int[] captainSalaries, int[] assistantSalaries, int[] sumOfCaptainSalaries) {
    int[][] dp = new int[2][n + 1];
    for (int i = 1; i <= n; i++) {
      dp[i & 1][i] = sumOfCaptainSalaries[n - 1] - sumOfCaptainSalaries[n - i - 1];
      //      dp[i & 1][0] = dp[!(i && 1)][1] + dp[n - i];
    }
    return captainSalaries[n - 1] + dp[n - 1][n / 2];
  }

  private static int charlieAndPilotsRecursive(
    int n,
    int[] captainSalaries,
    int[] assistantSalaries,
    int currentIndex,
    int diffOfAssistantsAndCaptains) {
    if (diffOfAssistantsAndCaptains < 0
        || n <= 0
        || diffOfAssistantsAndCaptains >= captainSalaries.length) {
    ...
    if (diffOfAssistantsAndCaptains == 0) {
      return assistantSalaries[currentIndex]
          + charlieAndPilotsRecursive(
              n - 1,
              captainSalaries,
              assistantSalaries,
              currentIndex + 1,
              diffOfAssistantsAndCaptains + 1);
    }
    if (diffOfAssistantsAndCaptains == n) {
      return captainSalaries[currentIndex]
          + charlieAndPilotsRecursive(
              n - 1,
              captainSalaries,
              assistantSalaries,
    ...
    int hiredAsAssistant =
        assistantSalaries[currentIndex]
            + charlieAndPilotsRecursive(
                n - 1,
                captainSalaries,
                assistantSalaries,
